...
void run_publisher_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::topic::Topic<LargeType> topic(participant, "Example LargeType");

    // Create a Publisher
    dds::pub::Publisher publisher(participant);

    // Create a DataWriter with default QoS
    dds::pub::DataWriter<LargeType> writer(publisher, topic);

    std::unique_ptr<LargeType> data(new LargeType());
    ...
    samples_written++) {
        ...
        data->id(1);
        data->values().at(0) = samples_written;
        data->values().at(1) = samples_written;
        data->values().at(2) = samples_written;

        std::cout << "Writing LargeType, count " << samples_written << std::endl;
