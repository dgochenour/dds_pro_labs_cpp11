...
int process_data(dds::sub::DataReader<LargeType> reader)
{
    // Take all samples
    int count = 0;
    dds::sub::LoanedSamples<LargeType> samples = reader.take();
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::sub::DataReader<LargeType> reader(subscriber, topic);
    ...
    while (!application::shutdown_requested && samples_read < sample_count) {
        std::cout << "LargeType subscriber sleeping up to 1 sec..." << std::endl;
