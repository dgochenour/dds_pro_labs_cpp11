...
int process_data(dds::sub::DataReader<SmallType> reader)
{
    // Take all samples
    int count = 0;
    dds::sub::LoanedSamples<SmallType> samples = reader.take();
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::sub::Subscriber subscriber(
            participant,
            dds::core::QosProvider::Default().subscriber_qos(qos_profile_name));

    dds::sub::DataReader<SmallType> reader(
            subscriber, 
            topic,
            dds::core::QosProvider::Default().datareader_qos(qos_profile_name));
    ...
    while (!application::shutdown_requested && samples_read < sample_count) {
        std::cout << "SmallType subscriber sleeping up to 1 sec..." << std::endl;
